...
async def _send_result(message: Message, payload: Dict[str, Any]) -> None:
    ...
    for p in pops:
        lines.append(f"📊 Исследование: {p.get('study', 'unknown')}")
        lines.append(
        ...
        if isinstance(gf, dict):
            lines.append("  Частоты генотипов (Hardy–Weinberg):")
            lines.append(f"    0/0: {gf.get('hom_ref'):.6f}")
            lines.append(f"    0/1: {gf.get('het'):.6f}")
            lines.append(f"    1/1: {gf.get('hom_alt'):.6f}")
        lines.append("")

    # Режем текст на чанки, чтобы не превысить лимит Telegram (~4096 символов)
    max_len = 3500  # немного запас
...
async def handle_plain_rsid(message: Message) -> None:
    # Любой текст без команды /get не обрабатываем как rsID
    await message.answer(
        "Я принимаю rsID только через команду /get.\n"
        "Используйте формат: /get rs12345\n"
        "Пример: /get rs1801133"
    )
...
async def main() -> None:
    ...
    dp.message.register(handle_plain_rsid, F.text)
